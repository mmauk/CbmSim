...
void Control::set_plasticity_modes(std::string pfpc_plasticity,
  ...
  // TODO: check if we have good evidence to implement diff plast types for mfnc
  // plast
...
void Control::initialize_session(std::string sess_file) {
  ...
  trialTime = std::stoi(
      s_file.parsed_var_sections["trial_spec"].param_map["trialTime"].value);
  msPreCS = std::stoi(
      s_file.parsed_var_sections["trial_spec"].param_map["msPreCS"].value);
  msPostCS = std::stoi(
      s_file.parsed_var_sections["trial_spec"].param_map["msPostCS"].value);
...
void Control::save_pfpc_weights_to_file(int32_t trial) {
  if (pfpc_weights_filenames_created) {
    ...
    if (trial != -1) // nonnegative indicates we want to append the trial to the
                     // file basename
    {
      curr_pfpc_weights_filename = data_out_path + "/" +
                                   get_file_basename(pfpc_weights_file) +
    ...
    std::fstream outPFPCFileBuffer(curr_pfpc_weights_filename.c_str(),
                                   std::ios::out | std::ios::binary);
...
void Control::save_mfdcn_weights_to_file(int32_t trial) {
  if (mfnc_weights_filenames_created) {
    ...
    if (trial != -1) // nonnegative indicates we want to append the trial to the
                     // file basename
    {
      curr_mfnc_weights_filename =
          data_out_path + "/" + get_file_basename(curr_mfnc_weights_filename) +
          "_TRIAL_" + std::to_string(trial) + BIN_EXT;
    }
    if (!simCore) {
      LOG_ERROR("Trying to write uninitialized weights to file.");
    ...
    std::fstream outMFDCNFileBuffer(curr_mfnc_weights_filename.c_str(),
                                    std::ios::out | std::ios::binary);
...
void Control::create_out_sim_filename() {
  if (data_out_dir_created) {
    out_sim_name = data_out_path + "/" + data_out_base_name + "_" +
                   get_current_time_as_string("%m%d%Y") + SIM_EXT;
...
void Control::create_out_info_filename() {
  if (data_out_dir_created) {
    out_info_name = data_out_path + "/" + data_out_base_name + "_" +
                    get_current_time_as_string("%m%d%Y") + TXT_EXT;
  ...
  if (data_out_dir_created) {
    for (uint32_t i = 0; i < NUM_CELL_TYPES; i++) {
      ...
      if (rast_map[cell_id] || use_gui) {
        rf_names[i] = data_out_path + "/" + data_out_base_name + "_" + cell_id +
                      "_RASTER_" + get_current_time_as_string("%m%d%Y") +
                      BIN_EXT;
      }
    }
  ...
  if (data_out_dir_created) {
    for (uint32_t i = 0; i < NUM_CELL_TYPES; i++) {
      ...
      if (psth_map[cell_id] || use_gui) {
        pf_names[i] = data_out_path + "/" + data_out_base_name + "_" + cell_id +
                      "_PSTH_" + get_current_time_as_string("%m%d%Y") + BIN_EXT;
      }
    }
  ...
  if (data_out_dir_created) {
    ...
    if ((pf_pc_plast != ABBOTT_CASCADE && pf_pc_plast != MAUK_CASCADE) &&
      ...
      pfpc_weights_file = data_out_path + "/" + data_out_base_name +
                          "_PFPC_WEIGHTS_" +
                          get_current_time_as_string("%m%d%Y") + BIN_EXT;
...
void Control::runSession(struct gui *gui) {
  ...
  while (trial < td.num_trials && run_state != NOT_IN_RUN) {
    ...
    // TODO: make sess file param for check if stp turned on,
    //       and make a trial frequency reset variable so this
    //       isn't hard-coded
    if (trial % 108 == 0) {
      simCore->getMZoneList()[0]->resetSTPDevice();
    }
...
void Control::save_rasters() {
  ...
  // std::string gr_elig_rast_name = data_out_path + "/gr_elig_rast_trial_" +
  // std::to_string(trial) + BIN_EXT; LOG_DEBUG("Saving gr eligibility raster to
  // file..."); write2DArray<float>(gr_elig_rast_name, gr_elig_raster,
  // msMeasure, num_gr); std::string pfpc_stp_rast_name = data_out_path +
  // "/pfpc_stp_rast_trial_" + std::to_string(trial) + BIN_EXT;
  // LOG_DEBUG("Saving pfpc stp raster to file...");
  // write2DArray<float>(pfpc_stp_rast_name, pfpc_stp_raster, msMeasure,
  // num_gr);
}
...
void Control::fill_rasters(uint32_t raster_counter, uint32_t psth_counter) {
  ...
  // const float *gr_elig_ptr =
  // simCore->getMZoneList()[0]->exportGREligToState(); if (gr_elig_ptr)
  //{
  ...
  // const float *pfpc_stp_ptr =
  // simCore->getMZoneList()[0]->exportPFPCSTPToState(); if (pfpc_stp_ptr)
  //{
  //	for (int i = 0; i < num_gr; i++)
  //	{
