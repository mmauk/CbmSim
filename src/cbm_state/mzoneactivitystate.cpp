...
void MZoneActivityState::resetState()
{
	initializeVals();
}
...
void MZoneActivityState::initializeVals()
{
	// stellate cells
	memset(apSC.get(), 0, num_sc * sizeof(uint8_t));
	memset(apBufSC.get(), 0, num_sc * sizeof(uint32_t));
	memset(gPFSC.get(), 0, num_sc * sizeof(float));
	std::fill(threshSC.get(), threshSC.get() + num_sc, threshRestSC);
	std::fill(vSC.get(), vSC.get() + num_sc, eLeakSC);

	// basket cells
	memset(apBC.get(), 0, num_bc * sizeof(uint8_t));
	memset(apBufBC.get(), 0, num_bc * sizeof(uint32_t));
	memset(inputPCBC.get(), 0, num_bc * sizeof(uint32_t));
	memset(gPFBC.get(), 0, num_bc * sizeof(float));
	memset(gPCBC.get(), 0, num_bc * sizeof(float));
	std::fill(vBC.get(), vBC.get() + num_bc, eLeakBC);
	std::fill(threshBC.get(), threshBC.get() + num_bc, threshRestBC);

	// purkinje cells
	// pc
	memset(apPC.get(), 0, num_pc * sizeof(uint8_t));
	memset(apBufPC.get(), 0, num_pc * sizeof(uint32_t));
	memset(inputBCPC.get(), 0, num_pc * sizeof(uint32_t));
	memset(inputSCPC.get(), 0, num_pc * sizeof(uint32_t));
	std::fill(pfSynWeightPC.get(), pfSynWeightPC.get()
		+ num_pc * num_p_pc_from_gr_to_pc, initSynWofGRtoPC);
	memset(inputSumPFPC.get(), 0, num_pc * sizeof(float));
	memset(gPFPC.get(), 0, num_pc * sizeof(float));
	memset(gBCPC.get(), 0, num_pc * sizeof(float));
	memset(gSCPC.get(), 0, num_pc * sizeof(float));
	...
	// inferior olivary cells
	memset(apIO.get(), 0, num_io * sizeof(uint8_t));
	memset(apBufIO.get(), 0, num_io * sizeof(uint8_t));
	memset(inputNCIO.get(), 0, num_io * num_p_io_from_nc_to_io * sizeof(uint8_t));
	memset(gNCIO.get(), 0, num_io * num_p_io_from_nc_to_io * sizeof(float));
	std::fill(threshIO.get(), threshIO.get() + num_io, threshRestIO);
	std::fill(vIO.get(), vIO.get() + num_io, eLeakIO);
	memset(vCoupleIO.get(), 0, num_io * sizeof(float));
	memset(pfPCPlastTimerIO.get(), 0, num_io * sizeof(int32_t));

	errDrive = 0;

	// nucleus cells
	memset(apNC.get(), 0, num_nc * sizeof(uint8_t));
	memset(apBufNC.get(), 0, num_nc * sizeof(uint32_t));
	memset(inputPCNC.get(), 0, num_nc * num_p_nc_from_pc_to_nc * sizeof(uint8_t));
	memset(inputMFNC.get(), 0, num_nc * num_p_nc_from_mf_to_nc * sizeof(uint8_t));
	memset(gPCNC.get(), 0, num_nc * num_p_nc_from_pc_to_nc * sizeof(float));
	std::fill(mfSynWeightNC.get(), mfSynWeightNC.get()
		+ num_nc * num_p_nc_from_mf_to_nc, initSynWofMFtoNC);
	memset(gMFAMPANC.get(), 0, num_nc * num_p_nc_from_mf_to_nc * sizeof(float));
	std::fill(threshNC.get(), threshNC.get() + num_nc, threshRestNC);
	std::fill(vNC.get(), vNC.get() + num_nc, eLeakNC);
	memset(synIOPReleaseNC.get(), 0, num_nc * sizeof(float));
