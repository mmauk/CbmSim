...
PoissonRegenCells::PoissonRegenCells(int randSeed, float threshDecayTau,
                                     unsigned int numZones, float sigma) {
...
const uint8_t *PoissonRegenCells::calcPoissActivity(const float *frequencies,
                                                    MZone **mZoneList,
                                                    int ispikei) {
  int countColls = 0;
 ...
 * depending on the sets of mfs, why calculate this every time step. waste of
 * time.
 */
bool *PoissonRegenCells::calcTrueMFs(const float *frequencies) {
  for (uint32_t i = 0; i < num_mf; i++) {
    if (frequencies[i] == -1) // indicates that this mf was not assigned a
                              // frequency, so is a dcn collateral
    {
      isTrueMF[i] = false;
    }
  }
...
const uint8_t *PoissonRegenCells::getAPs() { return (const uint8_t *)aps; }
