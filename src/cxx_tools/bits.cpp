...
// TODO: DEBUG
void pack_2d_byte_array(
    uint8_t **byte_arr_2d, const uint32_t byte_arr_num_rows, /* num cells */
...
void print_byte_bit_repr(uint8_t byte) {
  ...
  for (uint32_t i = 0; i < BITS_PER_BYTE; i++) {
    bit_flag &= byte >> i;
    if (bit_flag == '\001')
      byte_as_str[BITS_PER_BYTE - i - 1] = '1';
    else if (bit_flag == '\000')
      byte_as_str[BITS_PER_BYTE - i - 1] = '0';
    bit_flag = '\001';
  }
