...
void enable_raw_mode(FILE **fp) {
  ...
  tcgetattr(fileno(*fp),
            &term); /* get term bit flags from tty pointed to by fp */
  term.c_lflag &= ~(ICANON | ECHO); /* block term's canonical mode and echo */
  tcsetattr(fileno(*fp), TCSANOW,
            &term); /* update fp's flags to updated term's flags immediately */
}
...
void disable_raw_mode(FILE **fp) {
  ...
  term.c_lflag |=
      ICANON | ECHO; /* set term back to canonical mode and to echo */
  tcsetattr(fileno(*fp), TCSANOW, &term);
}

bool kbhit(FILE **fp) {
  int byteswaiting;
  ioctl(fileno(*fp), FIONREAD,
        &byteswaiting); /* get num immediately avail bytes in fp */
  return byteswaiting > 0;
}

/*
 * IMPLEMENTATION NOTE: on call, the read could return nothing. This fnctn
 * should only be called after a call to kbhit to ensure our tty has something
 * in it to actually read :>
 */
char getchar(FILE **fp) {
  ...
  if (tcsetattr(fileno(*fp), TCSANOW, &old) < 0)
    perror("tcsetattr ICANON");
  if (read(fileno(*fp), &buf, 1) < 0)
    perror("read()"); /* perform the read */
...
int reset_tty(FILE **fp) {
  ...
  tcflush(fileno(*fp),
          TCIFLUSH); /* flush input so characters don't show up later */
