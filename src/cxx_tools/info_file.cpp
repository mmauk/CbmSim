 ...
 *
 * Description:
 *     This file implements the prototypes given in info_file.h. In doing so it
 * also defines utility functions that are private only to this file.
 */
#include <ctime>
 ...
 *
 *     Uses the unix function 'geteuid' to query the effective user ID of the
 * calling process.
 */
 ...
 *     uses a string stream and std::put_time to format the current time
 * according to the default formats. The default formats include extra
 * wrapping-parentheses due to the ease with which this form can be handled when
 * actually writing to the info file. This choice may be subject to change.
 *
 *     The only failure state for this function is if the username could not be
 * obtained from get_user_name, for whatever reason.
 *
 */
void set_info_file_str_props(enum when when, info_file_data &if_data) {
  ...
  if (when == BEFORE_RUN) {
    ...
    if (user_name.empty()) {
      LOG_ERROR("Could not obtain username when setting information file "
                "properties.");
