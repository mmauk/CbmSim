 ...
 *     It includes the constants, enums, and functions required to parse the
 * command line arguments according to a two-mode scheme for building
 * simulations (i.e. creating new rabbits) and running simulations (i.e.
 * training rabbits).
 */
 ...
 * information here, in string representations. the raster, psth, and weights
 * files are maps from cell ids (or synapse ids) to booleans, which will be used
 * downstream to determine what data structures and filenames need to be
 ...
 *     invalid states that p_cl could be in cause the program to terminate.
 * However, some options (such as not specifying the visual mode) have default
 * values which are deferred to in cases in which their options are omitted
 * entirely. For a detailed list of the commandline options run the executable
 * with the '-h' flag.
 *
 */
...
/*
 * Description:
 *     a convenience wrapper for the functions 'parse_commandline' and
...
/*
 * Description:
 *     copies the data in 'from_p_cl' struct to 'to_p_cl' struct. Used only in
 * copying data to the contained instance in info_file (current as of
 * 12/21/2022)
 */
...
/* Description:
 *     overloads the stream insertion operator (<<) for the parsed_commandline
 * struct. With this overload, if you have a parsed_commandline called
 * p_cmdline, you can print its contents like so:
 *
