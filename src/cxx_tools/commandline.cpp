...
const std::vector<std::pair<std::string, std::string>> command_line_pair_opts 
{
	...
	{ "-a", "--altered-weights"}, // used to specify what file to use to overwrite weights
	...
	else
	{
		...
		else if (!p_cl.session_file.empty()) // checking validity of input for run mode
		{
			...
			if (!p_cl.altered_weights_file.empty())
			{
				...
				// NOTE: for now, assume that altered weights file will be in the data/outputs folder
				if (!file_exists(OUTPUT_DATA_PATH, p_cl.altered_weights_file, altered_weights_fullpath))
				{
			...
			if (!p_cl.weight_mask_file.empty())
			{
				...
				// NOTE: for now, assume that weights mask file will be in the data/outputs folder
				if (!file_exists(OUTPUT_DATA_PATH, p_cl.weight_mask_file, weight_mask_fullpath))
				{
