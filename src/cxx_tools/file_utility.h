 ...
 *
 * Description:
 *     Provides functions for serializing data to and from file. Serializiation
 * is the process of sending and retrieving binary data to and from file via a
 *     data structure of the programmer's choice. For now we implement a
 *     serialization interface for std::maps, but others can be added easily in
 * the future by utilizing the basic concepts which can be found in the
 * implementation file.
 *
 */
...
/*
 * Description:
 *     takes a reference to a string and a character and trims the character off
 * of the string from the left. The trimming is done in-place.
 */
inline void ltrim(std::string &s, unsigned char ch) {
  s.erase(s.begin(),
          std::find_if(s.begin(), s.end(),
                       [ch](unsigned char compare) { return compare != ch; }));
}

/*
 * Description:
 *     takes a reference to a string and a character and trims the character off
 * of the string from the right. The trimming is done in-place.
 */
 ...
 *     stripped of its path. If just the filename is given, it is returned
 * as-is. For example, if the file-path "/home/user/a_file" is given, then
 ...
 *     "/home/user/some_info.txt" is given, then "some_info" is returned as a
 * std::string.
 */
...
/*
 * Description:
 *     takes in a format string and returns the current time. The function is as
 * flexible as the time format allows, ie the time can be represented in
 * seconds, day/month/year, hour/minute/second, and so on.
 *
 *     For a detailed description of valid format strings, see
...
/*
 * Description:
 *     takes in a directory to search, a filename, and searches recursively for
 * the given filename. If a match is found, the full path of the filename is
 * saved in the reference resolved_fullpath and the function returns true.
 * Otherwise, the resolved_fullpath is set to the empty string and the function
 * returns false.
 */
bool file_exists(std::string dir_to_search, std::string &filename,
                 std::string &resolved_fullpath);

/*
 * Description:
 *     takes in a pointer (arr) and a given number of bytes (byteLen) and reads
 * from or writes to the fstream (file) if the given bool (read) is true or
 * false respectively.
 */
void rawBytesRW(char *arr, unsigned long byteLen, bool read,
                std::fstream &file);

/*
 * Description:
 *     takes in a map with template key type key_t and value type val_t and
 * writes the underlying bytes to file. The order is dictated by the
 * implementation of std::map. the first sizeof(size_t) bytes of the written
 * file represent the number of key-value pairs within the map.
 */
template <typename key_t, typename val_t>
void serialize_map_to_file(std::map<key_t, val_t> &map,
...
/*
 * Description:
 *     takes in a file and reads its contents into a map with template key type
 * key_t and value type val_t. The order is dictated by the implementation of
 * std::map. it is assumed that the data contained within the file is formatted
 * as key_t key and val_t val pairs, and can be cast into the types of key_t and
 * val_t respectively. It is also assumed that the first sizeof(size_t) bytes of
 * the file can be interpreted as the number of key-pair values contained within
 * the file.
 */
template <typename key_t, typename val_t>
void unserialize_map_from_file(std::map<key_t, val_t> &map,
                               std::fstream &file_buf) {
