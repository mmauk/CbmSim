...
static const std::string var_val_regex_str =
    "[+-]?([0-9]*[.])?[0-9]*([e][+-]?[0-9]+)?";

// look-up table for lexemes, is used for printing lexemes to whatever stream
// you want
...
/*
 * Implementation Notes:
 *     This function operates upon a tokenized file by going line-by-line,
 * token-by-token and matching the token with its lexical-definition. The only
 * tricky tokens are the variable identifiers and the variable values, as these
 * are naturally variable from one variable to the next. First I search for the
 * token as-is in the token definitions look-up table. This ensures that I'm not
 * matching a constant lexical token with a variable one. Then I attempt to
 * match, in sequence, the raw_token with the regex pattern string associated
 * with variable identifiers and variable values. Finally, I add the relevant
 * data into each lexed_token, add that lexed_token to l_file's tokens vector.
 *     Note that I add an "artificial" EOL token after each line has been lexed.
 * Used in parsing.
 */
    ...
    case VAR_IDENTIFIER:
      if (def_type == "trial") {
        if (prev_lex != TYPE_NAME) {
          ...
          curr_var.identifier = ltp->raw_token;
      } else {
      ...
      if (prev_lex != VAR_IDENTIFIER && prev_lex != NEW_LINE) {
        ...
        if (def_type == "trial") {
          ...
          if (def_type == "block") {
            ...
            // trial identifier (at a later stage of processing)
...
static void parse_var_section(std::vector<lexed_token>::iterator &ltp,
  ...
  while (ltp->lex != END_MARKER) {
    if (ltp->lex == TYPE_NAME) {
      ...
      if (next_lt->lex == VAR_IDENTIFIER && second_next_lt->lex == VAR_VALUE) {
        curr_var.type_name = ltp->raw_token;
...
static void parse_trial_section(std::vector<lexed_token>::iterator &ltp,
  ...
  while (ltp->lex != END_MARKER) // parse this section
  {
    if (ltp->lex == DEF) {
      auto next_lt = std::next(ltp, 1);
      auto second_next_lt = std::next(ltp, 2);
      if (next_lt->lex == DEF_TYPE && second_next_lt->lex == VAR_IDENTIFIER) {
        ltp += 4;
        parse_def(ltp, l_file, s_file, next_lt->raw_token,
                  second_next_lt->raw_token);
      } else {
      } // TODO: report error
    } else if (ltp->lex == SINGLE_COMMENT) {
      while (ltp->lex != NEW_LINE)
...
void initialize_trials_data(trials_data &td, parsed_trial_section &pt_section) {
  ...
  for (uint32_t i = 0; i < td.num_trials; i++) {
    td.use_css[i] =
        std::stoi(pt_section.trial_map[td.trial_names[i]]["use_cs"].value);
    td.cs_onsets[i] =
        std::stoi(pt_section.trial_map[td.trial_names[i]]["cs_onset"].value);
    td.cs_lens[i] =
        std::stoi(pt_section.trial_map[td.trial_names[i]]["cs_len"].value);
    td.cs_percents[i] =
        std::stof(pt_section.trial_map[td.trial_names[i]]["cs_percent"].value);
    td.use_uss[i] =
        std::stoi(pt_section.trial_map[td.trial_names[i]]["use_us"].value);
    td.us_onsets[i] =
        std::stoi(pt_section.trial_map[td.trial_names[i]]["us_onset"].value);
  }
}
 ...
 *     all *to_str functions use std::stringstreams to create a str
 * representation of the given data structure. For the std::ostream insertion
 * operator overloads, the *to_str functions are called to return the string
 * representation which is then inserted into the given argument std::ostream.
 *
 */
