...
void MZone::resetSTPDevice() {
  for (int i = 0; i < numGPUs; i++) {
    cudaSetDevice(i + gpuIndStart);
    cudaMemset(pfpcSTPsGPU[i], 0.0, numGRPerGPU * sizeof(float));
    cudaDeviceSynchronize();
  }
}
  ...
  for (int i = 0; i < num_io; i++) {
    ...
    for (int j = 0; j < num_p_io_from_nc_to_io; j++) {
      as->gNCIO[i * num_p_io_from_nc_to_io + j] =
          ...
          exp(-msPerTimeStep /
              (-gDecTSofNCtoIO *
                   exp(-as->gNCIO[i * num_p_io_from_nc_to_io + j] /
                       gDecTTofNCtoIO) +
               gDecT0ofNCtoIO));
      as->gNCIO[i * num_p_io_from_nc_to_io + j] =
          as->gNCIO[i * num_p_io_from_nc_to_io + j] +
...
void MZone::runPFPCGradedPlastCUDA(cudaStream_t **sts, int streamN,
  ...
  if (t % (uint32_t)tsPerHistBinGR == 0) {
    ...
    for (int i = 0; i < num_io; i++) {
      ...
      } else // this branch is never reached: the two above are mutually
             // exclusive and span all possibilities
      {
        pfPCPlastStepIO[i] = 0;
      }
    }
