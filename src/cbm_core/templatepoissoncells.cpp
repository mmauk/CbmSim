...
TemplatePoissonCells::TemplatePoissonCells(uint32_t num_gpus, std::fstream &psth_file_buf,
									 cudaStream_t **streams)
{
	...
	expansion_factor = 1; // by what factor are we expanding the granule cell number?
...
void TemplatePoissonCells::init_templates_from_psth_file(std::fstream &input_psth_file_buf)
{
	...
	uint32_t **input_psths = allocate2DArray<uint32_t>(num_in_template_ts, num_gr_old);
	...
	input_psth_file_buf.read((char *)input_psths[0], num_in_template_ts * num_gr_old * sizeof(uint32_t));
	LOG_INFO("finished load.");
	LOG_INFO("transposing...");
	uint32_t **input_psths_t = transpose2DArray(input_psths,  num_in_template_ts, num_gr_old);
...
void TemplatePoissonCells::calcGRPoissActivity(size_t ts, cudaStream_t **streams, uint8_t streamN)
{
	...
	callGRPoissActKernel(streams[i][streamN], calcGRActNumBlocks, calcGRActNumGRPerB,
