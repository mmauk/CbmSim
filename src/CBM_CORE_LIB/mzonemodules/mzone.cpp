...
{
//NOTE: these operations are done repeatedly, so
...
{
#ifdef DEBUGOUT
	std::cout << "resetting inputPCBC " << cp->numBC << std::endl;
#endif
	for (int i = 0; i < cp->numBC; i++)
	{
		as->inputPCBC[i] = 0;
...
{
	for (int i = 0; i < cp->numPC; i++) as->inputBCPC[i] = 0;
...
#ifdef DEBUGOUT
	std::cout << sumSynW / cp->numMF << std::endl;
#endif
}
...
void MZone::runPFPCOutCUDA(cudaStream_t **sts, int streamN)
{
	for (int i = 0; i < numGPUs; i++)
	{
		...
		callUpdatePFPCOutKernel(sts[i][streamN], updatePFPCNumBlocks, updatePFPCNumGRPerB,
				...
				inputPFPCGPUPitch[i], cp->numpPCfromGRtoPCP2);
	}
}
...
void MZone::cpyPFPCSumCUDA(cudaStream_t **sts, int streamN)
{
	for (int i = 0; i < numGPUs; i++)
	{
		...
		cudaMemcpyAsync(&inputSumPFPCMZH[cp->numPC * i / numGPUs], inputSumPFPCMZGPU[i],
				cp->numPC / numGPUs * sizeof(float), cudaMemcpyDeviceToHost, sts[i][streamN]);
	}
}
...
void MZone::testReduction()
{
	...
	std::cout << "numGPUs: "<< numGPUs << " numGRPerGPU: " << numGRPerGPU << std::endl;
	...
	std::cout << "NumPC per GPU: " << cp->numPC / numGPUs << std::endl <<
		"NumBC per GPU: " << cp->numBC / numGPUs << 
		"NUMSC per GPU: " << cp->numSC / numGPUs << std::endl;
